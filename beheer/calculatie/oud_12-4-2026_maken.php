...
// Versie: 4.2 - Volledige Versie (Meerdere Bussen + Uren Fix)
...
try {
    $instellingen = $pdo->query("SELECT * FROM calculatie_instellingen LIMIT 1")->fetch(); 
    $chauffeur_uurloon = $instellingen['uurloon_basis'] ?? 35.00; 
    
    // Haal bussen op en zet ze in een Javascript string voor het dynamisch toevoegen
    $bussen = $pdo->query("SELECT * FROM calculatie_voertuigen WHERE actief = 1 ORDER BY capaciteit ASC")->fetchAll(); 
            ...
            <div class="box-body">
                ...
                <div class="profit-grid"> 
                    ...
                    <div class="profit-box" style="background:#e3f2fd; border:1px solid #90caf9;"> 
                        ...
                        <input type="number" step="0.01" name="verkoopprijs" id="verkoopprijs" class="form-control" style="font-size:20px; text-align:center; margin-top:5px;" value="0"> 
...
<script>
setTimeout(() => {
    // 1. Bewaar jouw originele, perfect werkende rekenfunctie
    if(typeof window.rekenen === 'function') {
        window.oudeRekenen = window.rekenen;
    }

    // 2. Vervang hem door onze nieuwe paraplu-functie
    window.rekenen = function() {
        // Laat het originele script de uren en KM uitrekenen!
        if(typeof window.oudeRekenen === 'function') {
            window.oudeRekenen();
        }
        // Doe daarna de financiën voor de extra bussen
        customFinancieleBerekening();
    };
    
    // 3. Zorg dat de hoofdbus dropdown hem ook aanroept
...
function customFinancieleBerekening() {
    ...
    // Bereken de kosten voor de hoofdbus én alle extra bussen
    document.querySelectorAll('.bus-select-class').forEach(selectElement => {
        if(selectElement && selectElement.selectedIndex > 0) {
            ...
            // Wiskunde: (Kilometers * Prijs van deze specifieke bus) + (Uren * Uurloon Chauffeur)
